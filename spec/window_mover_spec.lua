describe("window_mover", function()
	...
	it("actionFirst の JinraiMode は候補を選んでもフォーカスウィンドウへ即時適用しない", function()
		local screen = newScreen(1, { x = 0, y = 0, w = 1200, h = 800 }, "uuid-a")
		local focused = newWindow(screen, { x = 100, y = 100, w = 200, h = 100 })
		local target = newWindow(screen, { x = 300, y = 200, w = 300, h = 200 })
		...
		local state, instance = newWindowMoverWithMock(options, focused, { focused, target })
		state.screens = { screen }

		instance.openWindowActionChooser({ jinraiMode = true })
		sendKey(state, "a")

		assert.are.equal("A", selectedCandidate.key)
		assert.are.equal(0, #focused.setFrameCalls)
		assert.is_true(instance.applyCandidateToWindow(selectedCandidate, target))
		assert.are.same({ x = 0, y = 0, w = 600, h = 800 }, target.setFrameCalls[1].frame)
		assert.are.equal(0, #focused.setFrameCalls)
	end)

	it("applyCandidateToWindow は action を明示したウィンドウへ適用する", function()
		local screen = newScreen(1, { x = 0, y = 0, w = 1200, h = 800 }, "uuid-a")
		local focused = newWindow(screen)
		local target = newWindow(screen)
		local _, instance = newWindowMoverWithMock({}, focused, { focused, target })

		assert.is_true(instance.applyCandidateToWindow({
			kind = "action",
			action = "closeWindow",
		}, target))
		assert.are.equal(0, focused.closeCalls)
		assert.are.equal(1, target.closeCalls)
	end)

	it("actionFirst は不一致入力から復帰した候補も即時適用せず通知する", function()
		local screen = newScreen(1, { x = 0, y = 0, w = 1200, h = 800 }, "uuid-a")
		local focused = newWindow(screen)
		local selectedCandidate
		local options = selectedAreaOptions({
		...
		local state, instance = newWindowMoverWithMock(options, focused, { focused })
		state.screens = { screen }
		...
		assert.are.equal("S", selectedCandidate.key)
		assert.are.equal(0, #focused.setFrameCalls)
	end)
