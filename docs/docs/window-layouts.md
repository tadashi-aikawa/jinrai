...
- `screen`(任意): 配置先ディスプレイのUUIDまたは`displayAliases`で定義した別名を指定します。省略時、またはそのディスプレイが未接続のときは、メインディスプレイに配置します。UUIDの確認方法は[Area Hints](area-hints.md#display-uuid)を参照してください。
