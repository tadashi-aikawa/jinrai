...
!!! note
    `focusBorder`、`windowHints`、`focusBack`、`windowMover`、`areaHints`、`applicationHints`は、
    設定ファイルにセクションを記述した機能だけが有効になります。

    `applicationHints.apps`だけはデフォルト値が空であり、有効化時に1件以上の指定が必要です。
    そのため、以下では`applicationHints`を省略してデフォルトの無効状態を示し、
...
```json
{
  ...
  "areaHints": {
    ...
    "defaultScreen": null,
    // ディスプレイUUIDごとのエリア名と選択キーです。ディスプレイ数ごとの分岐も指定できます。
    "screens": {},
    "actions": {
      // 選択中にウィンドウを閉じるキーです。
      "closeWindow": null,
...
## プロファイル(接続ディスプレイによる設定切り替え) {#profiles}
  ...
  "profiles": [
    {
      ...
      "displays": ["37D8832A-2D66-02CA-B9F7-8F30A301B230"],
      "overrides": {
        "jinraiMode": {
          "combo": {
...
| キー | 説明 |
| --- | --- |
| `displays` | ディスプレイUUIDの一覧です。いずれか1つでも接続されていればプロファイルが適用されます。大文字小文字は区別しません。 |
| `overrides` | 上書きする設定です。ルートと同じ構造で任意のセクションを指定できます（`$schema`と`profiles`を除く）。 |
...
ディスプレイUUIDの確認方法は[Area Hints](area-hints.md#ディスプレイuuidを確認)を参照してください。
...
`applicationHints`も同様ですが、有効化する場合は`apps`へ1件以上のアプリを指定してください。
...
`areaHints.screens`には、ディスプレイUUIDごとにエリア名と1〜3文字の選択キーを指定します。
