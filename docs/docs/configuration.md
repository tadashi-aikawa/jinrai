...
!!! note
    `focusBorder`、`windowHints`、`focusBack`、`windowMover`、`applicationHints`は、
...
```json
{
  ...
  "windowMover": {
    "commands": {
      ...
      // 移動先エリアの選択画面を開きます。
      "moveToSelectedArea": { "hotkey": { "modifiers": null, "key": null } },
      // JinraiModeで移動先エリアの選択画面を開きます。
      "moveToSelectedAreaInJinraiMode": { "hotkey": { "modifiers": null, "key": null } },
      // ウィンドウを最小化します。
    ...
    "selectedArea": {
      // screensに設定がないディスプレイで使うエリア名と選択キーのマップです。
      "defaultScreen": null,
...
`focusBorder`、`windowHints`、`focusBack`、`windowMover`は、設定ファイルにセクション（`{}`でも可）を記述すると有効になります。
セクションを記述しない場合は無効です。
