...
!!! note
    ...
    そのため、以下では`application_hints`を省略してデフォルトの無効状態を示し、
...
```json
{
  ...
  // "application_hints" は登録アプリが必須のため、デフォルトでは無効です。
}
```
...
有効にする場合は、次の全項目例を参考に`application_hints`セクションを追加してください。
...
`focus_border`、`window_hints`、`focus_back`、`window_mover`は、設定ファイルにセクション（`{}`でも可）を記述すると有効になります。
セクションを記述しない場合は無効です。
...
`macos_native_tabs.apps`は組み込みの対象（GhosttyとFinder）へ追加されます。
