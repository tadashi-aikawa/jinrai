...
フルスクリーンにしたウィンドウへ適用した場合は、フルスクリーンを自動で解除してから移動します。

選択画面は`escape`、候補外のクリック、または起動に使ったホットキーで閉じられます。

## ディスプレイUUIDを確認 {#display-uuid}

エリアが未設定の状態でArea Hintsを開くと、各ディスプレイ上にUUIDと設定例が表示されます。表示されたUUIDを`screens`のキーに使用します。
UUIDが表示されたメッセージをクリックすると、そのディスプレイのUUIDをクリップボードへコピーできます。
このUUIDは[ディスプレイ別名](display-aliases.md)の`displayAliases`、[プロファイル](profiles.md)の`displays`、Window Layoutsの`screen`にも使用できます。
...
| 設定 | 説明 |
| --- | --- |
| `screens` | ディスプレイUUIDまたは`displayAliases`で定義した別名ごとに、エリア名と選択キーを指定します。[ディスプレイ数ごとの分岐](#display-count-branches)も指定できます。 |
| `defaultScreen` | `screens`に設定がないディスプレイで使う、エリア名と選択キーのマップです。こちらもディスプレイ数ごとの分岐を指定できます。キーが他のディスプレイのキーと衝突する場合、`defaultScreen`側のキーには自動で数字プレフィックス(`2`, `3`, …)が付きます(例: `H`→`2H`)。 |
...
| `actions.<エリア名>` | 選択中にアクティブウィンドウを、アクティブウィンドウがあるディスプレイ内の該当エリアへ移動するキーです。[利用可能なエリア](window-mover-areas.md)のすべての名前を指定できます。表示中のディスプレイのエリアへ移動する`screens`と異なり、ラベルは表示されず、ウィンドウが今あるディスプレイを基準に移動します。 |
...
## ディスプレイ数で設定を切り替える {#display-count-branches}
...
## キーの制約 {#key-constraints}

- 選択キーは1〜3文字で指定します。
- 大文字と小文字は区別されます。大文字は **Shift+キー** を意味します。
    - `"k"` → kキーで発動
    - `"K"` → Shift+kで発動
    - `"kd"` → k→dの連続入力で発動
    - `"Kd"` → Shift+k→dの連続入力で発動
- 同じディスプレイ内のエリアキー、アクションキー、[Window Hints](window-hints.md)へ移るキーは重複させないでください。
- `"b"`と`"bd"`のような、一方がもう一方の先頭に一致する組み合わせも使用できません。
...
## JINRAI Modeで使う {#jinrai-mode}

[JINRAI Mode](jinrai-mode.md)では、Window Hintsによるウィンドウ選択とArea Hintsによる移動先選択を交互に繰り返します。

- `jinraiMode.hotkey`を設定すると、最初からJINRAI ModeとしてArea Hintsを開けます。
- 表示中に`jinraiMode.triggers.areaHints.key`([全設定](configuration.md)参照)を押すと、その場でJINRAI Modeを開始します。
...
JINRAI Mode中に`detachChromeTabToNewWindow`を実行した場合は、Window Hintsには戻らず、分離したChromeウィンドウを移動できるようにArea Hintsを再表示します。
