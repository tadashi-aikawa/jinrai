...
| `selectedArea.defaultScreen` | `screens`に設定がないディスプレイで使う、エリア名と選択キーのマップです。 |
...
エリア種類ごとの枠線色は固定です。アクティブウィンドウの強調は`selectedArea.activeWindowHighlight`(枠線色・太さ・角丸)と`selectedArea.activeWindowSpotlight.alpha`(暗幕の透明度)で変更できます。
