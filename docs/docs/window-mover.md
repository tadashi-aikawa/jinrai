...
`behavior.freeArea.excludeHiddenWindows`は、前面ウィンドウに隠れた背面ウィンドウを`freeArea`計算から除外するかを指定します。`false`にすると、完全に隠れているウィンドウも障害物として扱います。

`behavior.freeArea.hiddenWindowThreshold`は、`excludeHiddenWindows`が`true`のときに使う除外しきい値です。対象ウィンドウの画面内面積に対して、前面ウィンドウに隠れた割合がこの値以上なら除外します。`0`にすると、少しでも重なった背面ウィンドウを除外する旧挙動相当になります。
